...
\begin{itemize}
    ...
    \item Incorporate features that make it easy for companies and research groups to stay up to date with new features and bugfixes as well as continue contributing to the project
...
After transitioning to these more well known tools and improving our development practices, we have seen a rise in the number of community contributors and using gem5 has become less frustrating.
...
Many of gem5's features are also useful for other computer systems and programming languages research (e.g., as a platform for developing JIT compilers~\cite{}((Shingarov, "Live Introspection of Target-Agnostic JIT in Simulation"))).
...
Additionally, gem5 contains a duetailed in-order CPU model (the ``minor'' CPU) and an out-of-order CPU model (the ``O3'' CPU).
